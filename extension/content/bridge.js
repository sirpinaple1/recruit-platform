 ...
 * manifest 中本文件与 message-relay.js 同属一个 content_scripts 的 js 数组，
 * 二者被注入**同一个隔离世界**。两边原本都在顶层 `let contextDead`，后执行的
 * 这支直接抛 "Identifier 'contextDead' has already been declared"，bridge 整支
 * 没跑起来 —— 页面因此收不到 EXT_READY，表现为"检测不到插件"。
 * 包成 IIFE 后各文件自成一个作用域，同名不再互相干扰；今后新增 content script
 * 也请照此办理，不要向共享的顶层作用域声明任何东西。
 */
(() => {
  'use strict';
  ...
  const BRIDGE_ID = 'recruit-extension-bridge'; // window 上的注入标记（调试时可直接查）

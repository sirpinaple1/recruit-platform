...
| T4.5 | 管理端前端路线接通 | 授权/台账页面 UI 化 | ✅ 已完成（2026-09-16，recruit-web 补两个页面：①「扩展授权」页（/extension-tokens）：生成 token（名称校验）→ 明文一次性展示（警告条+select-all+复制按钮）→ 关闭后列表仅显 hash 前缀 → 吊销（confirm）；②「发布台账」页（/publish-records）：IPage 服务端分页 + 渠道/状态服务端筛选 + 单号/职位名客户端过滤 + 发布链接外链 + 状态 chip（pending/published/failed）；侧栏导航与路由同步接入。顺手修复时区不一致：JDBC URL 加 forceConnectionTimeZoneToSession=true（此前 DB DEFAULT CURRENT_TIMESTAMP 按 +08 生成、应用写入按 UTC，混合基准导致 DB 生成列前端显示 +8 偏差），修正 channel 存量时间；Chrome DevTools 端到端验收通过：UI 建需求→提交→审批→台账 pending 自动出现→UI 生成 token 可过扩展鉴权→吊销后 401→回填 published（链接/账号/发布时间正确展示）→状态/渠道筛选与关键字空态正确） |
